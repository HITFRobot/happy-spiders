...
class GezhoubaExcelPipeline(object):
    def __init__(self) -> None:
        self.file = os.path.join(data_dir, 'gezhouba.xlsx')
        self.excel = load_workbook(self.file)

    def process_item(self, item, spider):
        if item['name'] != 'gezhouba':
            return item
        year = item['time'].split('-')[0]
        # 获取指定的sheet
        sheet = self.excel.get_sheet_by_name(year)
...
class XiluoduExcelPipeline(object):
    def __init__(self) -> None:
        self.file = os.path.join(data_dir, 'xiluodu.xlsx')
        self.excel = load_workbook(self.file)
